...
namespace LeiaLoft.LeiaConfigUI
{
    public class EyeTrackingSettingsPanel : MonoBehaviour
    {
        ...
        [SerializeField] private Toggle BlackViewsOverrideToggle;
        [SerializeField] private Toggle BlackViewsOverrideValueToggle;
        ...
        void UpdateLabelsAndSliders()
        {
            ...
            if (BlackViewsOverrideToggle != null)
            {
                BlackViewsOverrideToggle.SetIsOnWithoutNotify(LeiaDisplay.Instance.BlackViewsOverride);
            }
            if (BlackViewsOverrideValueToggle != null)
            {
                if (LeiaDisplay.Instance.BlackViewsOverride)
                {
                    BlackViewsOverrideValueToggle.SetIsOnWithoutNotify(LeiaDisplay.Instance.BlackViewsOverrideValue);
                }
                else
                {
                    BlackViewsOverrideValueToggle.SetIsOnWithoutNotify(LeiaDisplay.Instance.blackViews);
                }
                BlackViewsOverrideValueToggle.interactable = LeiaDisplay.Instance.BlackViewsOverride;
            }
    ...
    if (DelaySlider != null)
    {
        ...
        void Update()
        {
            ...
            {
                FaceXInputField.SetTextWithoutNotify("" + LeiaDisplay.Instance.viewerPositionPredicted.x);
                FaceYInputField.SetTextWithoutNotify("" + LeiaDisplay.Instance.viewerPositionPredicted.y);
                FaceZInputField.SetTextWithoutNotify("" + LeiaDisplay.Instance.viewerPositionPredicted.z);
            }
        }
        ...
        public void SetEyeTrackingEnabled(bool enabled)
        {
            ...
            {
                LeiaDisplay.Instance.viewerPositionPredicted.x = 0;
                LeiaDisplay.Instance.viewerPositionPredicted.y = 0;
                LeiaDisplay.Instance.viewerPositionPredicted.z = LeiaDisplay.Instance.GetDisplayConfig().ConvergenceDistance;
        ...
        public void SetBlackViewsValue(bool enabled)
        {
            LeiaDisplay.Instance.BlackViewsOverrideValue = enabled;
            leiaPersistentSettings.SaveSettings();
            UpdateUI();
        }

        public void SetBlackViewsOverride(bool enabled)
        {
            LeiaDisplay.Instance.BlackViewsOverride = enabled;
            leiaPersistentSettings.SaveSettings();
            BlackViewsOverrideValueToggle.interactable = enabled;
            UpdateUI();
        }
