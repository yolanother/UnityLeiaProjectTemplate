...
namespace LeiaLoft.LeiaConfigUI
{
    public class LeiaRenderSettingsPanel : MonoBehaviour
    {
        ...
        [SerializeField] private Dropdown SetTrackingBackendDropdown;
        ...
        void Start()
        {
            ...
            EyeTrackingAndroid tracker = (EyeTrackingAndroid)LeiaDisplay.Instance.tracker;
            if (tracker.FaceTrackingBackend == Leia.FaceDetectorBackend.CPU)
            {
                CurrentTrackingBackend.text = "Current Backend: CPU";
            }
            else if (tracker.FaceTrackingBackend == Leia.FaceDetectorBackend.GPU)
            {
...
#if UNITY_ANDROID && !UNITY_EDITOR
        SetTrackingBackendDropdown.onValueChanged.AddListener(delegate{OnSetTrackingBackendDropdownValueChanged();});
#endif
        }
...
#if UNITY_ANDROID && !UNITY_EDITOR
    void OnSetTrackingBackendDropdownValueChanged()
    {
        if (SetTrackingBackendDropdown.value == 1)
        {
            Debug.Log("Setting tracking backend to GPU");
            SetGPU();
        }
        else
        {
            Debug.Log("Setting tracking backend to CPU");
            SetCPU();
        }
    }
