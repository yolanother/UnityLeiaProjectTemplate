...
namespace LeiaLoft
{
    ...
    public abstract class AbstractLeiaStateTemplate : ILeiaState, IReleasable
    {
        ...
        public virtual void DrawImage(LeiaCamera camera, LeiaStateDecorators decorators)
        {
            ...
            _material.SetFloat("faceX", LeiaDisplay.Instance.viewerPositionPredicted.x);
            _material.SetFloat("faceY", LeiaDisplay.Instance.viewerPositionPredicted.y);
            _material.SetFloat("faceZ", LeiaDisplay.Instance.viewerPositionPredicted.z);
            ...
            if (trackerIsOn
                    ...
                    && numViews >= 5)
            {
                float range = Mathf.Max(2.0f * 65.0f * d_over_n * p_over_du / pixelPitch / LeiaDisplay.Instance.tracker.GetPredictedFacePosition().z,6.0f);
