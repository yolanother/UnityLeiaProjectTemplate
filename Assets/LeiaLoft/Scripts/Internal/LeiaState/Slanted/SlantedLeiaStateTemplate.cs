...
namespace LeiaLoft
{
    ...
    public class SlantedLeiaStateTemplate : AbstractLeiaStateTemplate
    {
        ...
        public override void UpdateViews(LeiaCamera leiaCamera)
        {
            ...
            for (int ny = 0; ny < _viewsHigh; ny++)
            {
                for (int nx = 0; nx < _viewsWide; nx++)
                {
                    ...
                    float posx = GetPosX(leiaCamera, baseline, nx, ny);
                    float posy = GetPosY(leiaCamera, baseline, nx, ny);

                    // must set position before calculating projection-for-position
                    view.Position = new Vector3(posx, posy, leiaCamera.CameraShift.z);

                    m = CameraCalculatedParams.GetConvergedProjectionMatrixForPosition(
                        view.Camera, 
                        leiaCamera.transform.position + leiaCamera.transform.forward * leiaCamera.ConvergenceDistance
                    );
        ...
        float GetPosX(LeiaCamera leiaCamera, float baseline, int nx, int ny)
        {
        ...
        float GetPosY(LeiaCamera leiaCamera, float baseline, int nx, int ny)
        {
        ...
        public override void UpdateState(LeiaStateDecorators decorators, ILeiaDevice device)
        {
            if (_material == null)
            {
                _material = CreateMaterial(decorators.MultiCameraComposit);
            }
