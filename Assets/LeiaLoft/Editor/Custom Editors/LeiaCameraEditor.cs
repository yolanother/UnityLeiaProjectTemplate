...
namespace LeiaLoft
{
    ...
    public class LeiaCameraEditor : UnityEditor.Editor
    {
        ...
        private const string NaturalCameraFOVDocumentationLink = ""; //TODO: Insert documentation link here
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_controller.Camera.fieldOfView != 20)
            {
                ...
                if (GUILayout.Button("Set Natural Camera FOV"))
                {
                    ...
                    //set new fov
                    _controller.Camera.fieldOfView = 20;

                    //calculate new convergence distance and camera position
                    float newDistance = displayHalfHeight / Mathf.Tan((_controller.Camera.fieldOfView / 2f) * Mathf.Deg2Rad);
                    //set new camera position
                    _controller.transform.position = displayPosition - _controller.transform.forward * newDistance;
                    _controller.ConvergenceDistance = newDistance;
                    SetCameraFOVButtonPressed = true;
                }
                if (GUILayout.Button("Learn More"))
                {
                    Application.OpenURL(NaturalCameraFOVDocumentationLink);
                }
            }
            else
            {
                if (SetCameraFOVButtonPressed)
                {
                    GUILayout.Label("Keep new settings?");
                    ...
                    {
                        SetCameraFOVButtonPressed = false;
                    }
                }
            }
